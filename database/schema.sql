...
CREATE TABLE `p_company` (
    ...
    -- 业务详情
    `main_products` VARCHAR(500) COMMENT '主营产品',
    `main_market` VARCHAR(200) COMMENT '主要销售市场',
    `import_ability` INT COMMENT '进口能力(0-25)',
    `purchase_scale` INT COMMENT '采购规模(0-25)',
    `china_supplier_acceptance` INT COMMENT '中国供应商接受度(0-20)',
    `oem_aftermarket_match` INT COMMENT 'OEM/Aftermarket匹配度(0-15)',
    `export_ability` INT COMMENT '出口能力(0-10)',
    `customization_match` INT COMMENT '定制需求匹配度(0-5)',
    `quality_requirement` VARCHAR(100) COMMENT '质量要求',
    `price_sensitivity` VARCHAR(100) COMMENT '价格敏感度',
    `delivery_requirement` VARCHAR(100) COMMENT '交期要求',
    `accept_china_factory` VARCHAR(50) COMMENT '接受中国工厂',
    `customization_ability` VARCHAR(200) COMMENT '定制能力需求',
    `after_sales_requirement` VARCHAR(200) COMMENT '售后要求',
    `supply_chain_pain_points` TEXT COMMENT '当前供应链痛点',
    `recommended_products` VARCHAR(500) COMMENT '推荐切入产品',
    `recommended_channels` VARCHAR(200) COMMENT '推荐渠道',
    `first_email_strategy` TEXT COMMENT '首封开发信策略',
    `follow_up_immediately` VARCHAR(50) COMMENT '是否立即跟进',
    `added_to_crm` VARCHAR(50) COMMENT '加入CRM',
    `manual_follow_up` VARCHAR(50) COMMENT '人工跟进',
    `remarketing` VARCHAR(50) COMMENT '再营销',
    `development_email` TEXT COMMENT '开发信(英文)',
